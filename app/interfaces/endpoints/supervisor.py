...
async def cancel_timeout(
    ...
    return Response.success(
        msg=f"超时销毁已取消" if result.status == "timeout_canceled" else "超时销毁未设置",
        data=result,
    )
...
async def timeout_status(
    ...
    remaining_minutes = int((result.remaining_seconds or 0) // 60)
    msg = "未激活超时销毁" if not result.active else f"剩余超时销毁分钟数: {remaining_minutes}"
    return Response.success(
        msg=msg,
        data=result,
    )
